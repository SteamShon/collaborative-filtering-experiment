 ...
 */
public class UploloadToHbaseTableJob {

  private static final String NAME = "SampleUploader";
  
  static class Uploader extends //Mapper<LongWritable, Text, ImmutableBytesWritable, Put> {
    Mapper<IntWritable, VectorWritable, ImmutableBytesWritable, Put> {
    

    private long checkpoint = 100;
    private long count = 0;
    private byte[] family = Bytes.toBytes("vector");
    ...
    public void map(IntWritable key, VectorWritable value, Context context)
      ...
      while (iter.hasNext()) {
        ...
        byte[] qualifier = Bytes.toBytes(e.index());
        byte[] v = Bytes.toBytes(e.get());
        put.add(family, qualifier, v);
      }
